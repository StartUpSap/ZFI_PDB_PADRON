CLASS zcl_isupplieritr DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  PUBLIC SECTION.
  ...
  PRIVATE SECTION.
    ...
    TYPES ty_response_tab TYPE STANDARD TABLE OF zc_i_supplier_itr WITH EMPTY KEY.
    ...
    "METHODS if_rap_query_provider~select.
...
CLASS ZCL_ISUPPLIERITR IMPLEMENTATION.


  METHOD if_rap_query_provider~select.
*    TRY.
*        CASE io_request->get_entity_id( ).
*          WHEN 'ZC_I_SUPPLIER_ITRV2'.
...
***select data
*              DATA lt_travel_response TYPE STANDARD TABLE OF zc_i_supplier_itr."/dmo/i_travel_uq.
**              SELECT (lv_req_elements) FROM zc_i_supplier_itr "/dmo/travel
*              SELECT * FROM zc_i_supplier_itr "/dmo/travel
**              WHERE (lv_sql_filter)
**              GROUP BY (lv_grouping)
**              ORDER BY (lv_sort_string)
**              INTO CORRESPONDING FIELDS OF TABLE @lt_travel_response
*              ORDER BY Supplier
*              INTO   TABLE @lt_travel_response
*              OFFSET  @lv_offset
...
***select count
*              SELECT COUNT( * ) FROM zc_i_supplier_itr "/dmo/travel
*              "WHERE (lv_sql_filter)
